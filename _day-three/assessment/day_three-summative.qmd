...
-   Use the base `{dplyr}` verbs (i.e., `filter`, `select`, `mutate`), adverbs (i.e., `across`, `where`, `starts_with`, `ends_with`, `contains`, `matches`, `all_of`, `any_of`), and constructors (i.e., `tibble`, and `tribble`). [@R-dplyr]
:::
