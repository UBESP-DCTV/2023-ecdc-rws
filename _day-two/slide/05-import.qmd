...
```{r}
class(mtcars_tbl) # `tibble`s are `data.frame`s!
```
...
You can directly save a multi-sheet Excel file writing a list of dataframes...
...
... and import multiple sheet data in a (list of) dataframe(s)
...
-   leading headers's groups
-   empty columns and rows
-   multi-line heders
-   space and special charachters in names
-   missing information
-   repeated-data reported at first occurence only
:::
...
::: callout-caution
## What's next
We are quite to the end, we only need to make consistent convention for missing information. They are both reported as empty (alredy parsed as `NA`, correctly) and with `??`. We need to tell `rio::import` that also them are missing information
:::
...
::: {.fragment .fade-in fragment-index="1"}
::: callout-important
Most often you will not have to manage multi-row header nor implicit information. Depending on your data, missing information can be coded not with simple empty cells, so that you would need to set `na = c()` explicitely. On the other hand, the pattern
...
- [janitor](https://sfirke.github.io/janitor/) pakege.
