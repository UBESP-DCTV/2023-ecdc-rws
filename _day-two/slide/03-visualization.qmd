...
1.  Quite unreadable and extremely difficoult to follow; mainly because each argument is at a increasing distance from its function names (e.g., `multiply_by` is the first name appearing, and its second argument `h` is the last one!)
