...
### 4. \[50'\] Import and cleaning

-   \[20'\] Base and Tidy, and `rio`[^5] [^6]
-   \[30'\] Headers, var names, and missing data[^7]

::: {.callout-note appearance="minimal"}
\[10'\] BREAK
:::

### 5. (a) \[55'\] Transform -- manage shape[^8]

-   \[55'\] Tidy format & pivoting

::: {.callout-note appearance="minimal"}
\[10'\] BREAK
:::

### 5. (b) \[45'\] Transform -- manage dataset[^9]

-   \[15'\] select (all_of...)

-   \[15'\] filter (.data & .env)

-   \[15'\] mutate (grouped) data contents & across

### 5. (c) \[45'\] Transform -- manage formats[^10]
...
### 6. \[90'\] Modeling (summary statistics tables)[^11]
...
### 7. \[60'\] Communicate (tools: R markdown and Quarto)[^12]
...
[^3]: pkgs: `{magrittr}`, `{ggplot2}`

[^4]: pkgs: `{magrittr}`, `{ggplot2}`
...
[^8]: pkgs: `{janitor}`, `{tidyr}`

[^9]: pkgs: `{dplyr}`

[^10]: pkgs: `{dplyr}`, `{lubridate}`, `{stringr}`, `{glue}`

[^11]: pkgs: `{dplyr}`, `{gtsummary}`

[^12]: The *Communicate* section is flexible. Topics from this section can be omitted to allow for more in-depth discussion on earlier sections, if needed, based on the time available.
