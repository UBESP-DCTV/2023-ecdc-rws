...
```{r}
#| output-location: column
# Code to the right is evaluate, then 
...
::: callout-important
Variable names can contain letters, numbers, underscores and periods but no spaces. They must start with a letter (or a single dot, for hidden variables).
:::
:::
...
::: callout-tip
During the course we will see, explain and use `tibbles`: a modern, enhanced, better displayed, and with stricter and more consistent structure than standard data frames.
:::
:::
...
1.  Before to evaluate it, in the pad, under the section `1.3. Ex3`, write (in a new line) what is your expected result from the following computation:
