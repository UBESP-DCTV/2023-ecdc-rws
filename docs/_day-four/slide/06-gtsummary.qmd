...
-   Merge multiple tables horizontally to compare or combine data
-   Stack multiple tables vertically to compare or combine data
:::
:::
...
## `tbl_summary()` - Main arguments
  ...
  - `by` - the variable(s) to stratify the table by, i.e., the variable(s) that define the _summary_ columns of the table. Summary statistics will be calculated separately for each level of the by variable (e.g. by = `trt`). 
...
1.  Under the section `4.2. Ex26`, answer to the following question with an x next to the correct answer.

Which argument of `tbl_summary` you should use to select the variables to include in the table?
...
2. Under the section `4.2. Ex27`, answer to the following question with an x next to the correct answer.

Which argument of `tbl_summary` you should use to stratify the table by a variable?
...
ME: Connect to the Day-4 project in RStudio cloud (<https://bit.ly/ubep-rws-rstudio>): script `17-gtsummary.R`
...
1.   Under the section `4.2. Ex28`, answer to the following question with an x next to the correct answer.

Can you use tidyverse selectors to select variables to include in the table, i.e., as the `include` argument of `tbl_summary`?

2.  Then, open the script `18-tbl_summary.R` and follow the instruction step by step.
...
1.   Under the section `4.2. Ex29`, answer to the following question with an x next to the correct answer.

Can you pass two vectors to the `row` and `col` arguments of `tbl_cross` to create a cross-table?
...
# Tables for univariable models - multiple regression at once 

## `tbl_uvregression()` - base {auto-animate=true}
...
## `tbl_uvregression()` - autodetect {auto-animate=true}
...
1.  Under the section `4.2. Ex30`, answer to the following question with an x next to the correct answer.

What do you pass in the method argument of `tbl_uvregression`, a model object or a function?

model object
function


2.  Then, open the script `20-tbl_uvregression.R` and follow the instruction step by step.
...
# Merging (h) and stacking (v) tables
...
1.  Under the section `4.2. Ex31`, answer to the following question with an x next to the correct answer.

What happens if you pass two tables with different (but same number of) variables  to `tbl_merge`?

- error
- the tables are merged with missing values
- the table wrongly merges the variables

2.  Then, open the script `21-merge.R` and follow the instruction step by step.
...
[10 minutes](https://apps.garrickadenbuie.com/countdown/?_inputs_&time=%2220%3A00%22&update_every=%221%22&warn_time=%2204%3A00%22)[^1]

[^1]: <https://apps.garrickadenbuie.com/countdown>
