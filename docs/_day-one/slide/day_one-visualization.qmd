...
::: callout-important
All geometry functions are called `geom_*`, with `*` indicating the type of geom. `?geom_point`, `?geom_line`, `?geom_bar`, `?geom_boxplot`, `?gome_histogram`, ...
:::
:::
:::
...
Suppose we would like to have the bars filled in blue?\[\^6\]

We can add the parameter within the `geom_*` directly.
...
Why the following code produce red bars, and the legend report "fill" as header and "blue" as level?
...
::: callout-tip
For having a blue bar chart, put the parameter, within the corresponding layer (e.g., the `geom_*`, or globally in the `ggplot` call), **out of the `aes` call**! These are actually aesthetics, used to set them to a fixed value, like `colour = "red"` or `size = 3`, instead of mapping data to the aesthetics!
:::
:::
:::
:::
...
We can also *add* multiple geometrics one *on top* of the others. In which case, it could be useful to set personalized `aes`thetics abd customized the position of the geoms.
...
::: callout-important
We would like to also set the *position* of the `grom_*` we are creating. Look at the documentation to find which position are supported by your `geom_*`.
:::
:::
...
::: callout-tip
Most common positions are `"stack"` (default, multiple bars occupying the same `x` position will be stacked atop one another ), `"dodge"` (dodged side-to-side), `"fill"` (shows relative proportions at each x by stacking the bars and then standardising each bar to have the same height)
:::
:::
:::
...
### Global mapping from data to aesthetic
...
### Local mapping from data to aesthetic
